...
public class PartialSerializer {
	...
	protected class SerializeRecursiveStrategy implements SerializationStrategy {
		...
		// public SerializeRecursiveStrategy(EObject root, ISerializationContext
		// context) {
		// this(null, root, context);
		// }
	...
	protected List<SerializationStrategy> trySerializeMultiValue(EObject owner, FeatureChange change,
		...
		// List<ISemanticRegion> featureRegions =
		// ownerRegion.getRegionFor().features(feature);
		// List<Object> newValues = Lists.newArrayList();
		// switch (listTransient) {
		// case YES:
		// for (ISemanticRegion region : featureRegions) {
		// result.add(new DeleteRegionStrategy(region));
		// }
		// break;
		// case SOME:
		...
		// if (!transientValues.isValueInListTransient(owner, i, feature)) {
		// newValues.add(values.get(i));
		// }
		// }
		// break;
		// case NO:
		// newValues.addAll((List<?>) owner.eGet(feature));
		// }
		return result;

	}

	// protected List<SerializationStrategy>
	// trySerializeMultiValueContainment(EObject owner, FeatureChange change,
	// IEObjectRegion ownerRegion, IConstraint constraint) {
	// EStructuralFeature feature = change.getFeature();
	// List<SerializationStrategy> result = Lists.newArrayList();
	// IFeatureInfo featureInfo =
	// constraint.getFeatures()[owner.eClass().getFeatureID(feature)];
	// List<IConstraintElement> assignments = featureInfo.getAssignments();
	// if (assignments.size() != 1) {
	// return null;
	// }
	// // boolean optional = assignments.get(0).isOptional();
	// ListTransient listTransient = transientValues.isListTransient(owner,
	// feature);
	// List<ISemanticRegion> featureRegions =
	// ownerRegion.getRegionFor().features(feature);
	// List<Object> newValues = Lists.newArrayList();
